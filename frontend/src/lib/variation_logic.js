...
3. 빈칸/밑줄/보기 문법 골격 고정 및 [의도적 문법 오류 강제 보존]: 문법 출제 포인트가 되는 구조는 반드시 고정하세요.
★★ [핵심 경고] AI가 문장을 매끄럽게 만들고자 하는 본능 때문에 어법상 틀린 문장(오답)을 정답으로 자동 교정해버리는 치명적인 문제가 자주 발생합니다!
원본(original_passage)에서 대상이 되는 특정 형태(밑줄, 기호 등)가 "일부러 틀리게 작성된 문법(오답)"이라면, 변형된 문장에서도 반드시 "정확히 동일한 유형의 문법 오류"를 일부러 저질러서(강제로 파괴해서) 작성해야 합니다. 절대 올바른 문장으로 고치지 마세요! (예: 원본이 'He don't like appples'(직설법 3인칭 단수 오류) 라면 변형 지문도 'She don't like bananas' 와 같이 똑같이 파괴할 것)
...
6. 철저한 난이도 및 분량 통제 (Vocabulary Level Control)
현재 지정된 난이도: [${difficulty}]
- 원본의 분량(±10% 이내)을 유지하세요.
- [경고] 어휘 수준을 타겟 학년에 맞추어 극도로 제한하세요!
- 중학교 수준(중1~중3)의 경우 반드시 CEFR A2 ~ B1 초반 수준의 쉬운 어휘만 사용해야 합니다. 
- (예: 중1 수준에 mesmerize, simultaneously, protagonist 같은 단어 사용 절대 금지. 대신 amaze, at the same time, main character 등으로 치환)
- 지정된 난이도를 벗어나는 너무 어려운 어휘가 포함되면 안 됩니다.

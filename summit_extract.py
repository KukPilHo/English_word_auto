...
규칙:
- 보기 번호 ①②③④⑤, 지문 속 마커 ⓐⓑⓒⓓⓔ, (A)(B)(C), 밑줄을 그대로 보존한다. 밑줄은 <u>...</u> 로 표기.
- question: 발문(번호 포함 가능). passage: 지문/대화가 있으면 원문, 없으면 "".
- options: 보기 배열(예 "① ..."). 서술형 등 보기가 없으면 [].
...
- 보기 번호 ①②③④⑤, 마커 ⓐⓑⓒⓓⓔ, (A)(B)(C) 를 보존한다. (텍스트 추출본에는 밑줄 정보가 없을 수 있다 — 있으면 <u>...</u>.)
- question: 발문(번호 포함 가능). passage: 지문/대화가 있으면 원문, 없으면 "".
- options: 보기 배열. 없으면 [].
...
def run_pipeline(pdf_path, grade, dpi=200, split_columns=True, page_limit=None,
                 verbose=False):
